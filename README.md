...
To verify the integrity of the `nanoid` source, first download the target version and its signature file from the [releases page](https://github.com/sixafter/nanoid-cli/releases) along with its `.sig` file; e.g., `nanoid-1.32.0.tar.gz` and `nanoid-1.32.0.tar.gz.sig`. Then run the following command to verify the signature:
